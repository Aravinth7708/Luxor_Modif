...
export default function EnhancedBookingForm({
  ...
  const checkAuthenticationForBookingStep = () => {
    if (!authToken) {
      const currentBasePrices = calculateBasePrice()
      ...
      const currentBookingData = {
        ...
        originalAmount: currentBasePrices.originalPrice,
        finalAmount: currentBasePrices.finalPrice,
        hasOffers: currentBasePrices.hasOffers,
    ...
    try {
      ...
      for (let i = 0; i < nights; i++) {
        ...
        if (pricingWithOffers[dateKey]) {
          ...
          dayPrice = getPriceForDate(currentDate, villa)
        }
      ...
      const finalTotal = Math.round(totalPrice + serviceFee + taxes)
  ...
  const handleDateChangeWithTime = (startDate, endDate) => {
    if (onDateChange) {
  ...
  const isAddressComplete = (addr) => {
    return !!(addr.street && addr.country && addr.state && addr.city && addr.zipCode)
  }
  ...
  useEffect(() => {
    if (initialAddress.street || initialAddress.country || initialAddress.city) {
      ...
      setHasAddressData(isAddressComplete(initialAddress))
    }
  }, [initialAddress])
  ...
  useEffect(() => {
    setHasAddressData(isAddressComplete(address))
  }, [address])
  ...
  const fetchUserAddressInfo = async () => {
    try {
      if (initialAddress.street || initialAddress.country || initialAddress.city) return
      const API_BASE_URL = "https://luxorfinalbackend.onrender.com"
      const token = localStorage.getItem("token")
      if (!token) return
      const response = await axios.get(`${API_BASE_URL}/api/bookings/user-address-info`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (response.data && response.data.success && response.data.addressInfo) {
        setAddress(response.data.addressInfo)
        setHasAddressData(isAddressComplete(response.data.addressInfo))
        if (response.data.addressInfo.country) {
          handleCountryChange(response.data.addressInfo.country)
          if (response.data.addressInfo.state) {
            handleStateChange(response.data.addressInfo.country, response.data.addressInfo.state)
          }
        }
      }
    } catch (e) { }
  ...
  const fetchCountries = async () => {
    ...
    try {
      const defaultIndia = { name: "India", code: "IN" }
      const response = await fetch("https://countriesnow.space/api/v0.1/countries/positions")
      const result = await response.json()
      if (result.data && Array.isArray(result.data)) {
        let countriesList = result.data.map(c => ({ name: c.name, code: c.iso2 || "" })).sort((a, b) => a.name.localeCompare(b.name))
        countriesList = countriesList.filter(c => c.name.toLowerCase() !== "india")
        const allCountries = [defaultIndia, ...countriesList]
        setCountries(allCountries)
      } else {
        setCountries([defaultIndia])
      }
    } catch (e) {
  ...
  const handleStateChange = async (countryName, stateName) => {
    ...
    if (isSignedIn) {
      ...
      try {
        ...
        try {
          ...
          if (response.data && response.data.success && response.data.addressInfo) {
            const addr = response.data.addressInfo
            const list = [{ id: "saved", label: "Saved Profile Address", ...addr }]
  ...
  const checkUserPhone = async () => {
    try {
      ...
      if (!token) return
      const response = await axios.get(`${API_BASE_URL}/api/profile/phone-status`, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  return (
    <div
      className={`${isModal ? "relative" : ""} ${!isModal ? "lg:sticky lg:top-[calc(4rem+2rem)]" : ""
        } transition-all duration-300`}
    >
      <div
        className={`bg-white rounded-2xl border border-gray-200 p-4 lg:p-6 ${isModal ? "mt-0" : "mt-4"
      ...
      >
        ...
        <div className="space-y-4">
          {bookingStep === 1 && (
            <div className="space-y-3">
              {/* Airbnb-style Date & Guest Selector Box */}
              <div className="border border-gray-400 rounded-xl overflow-hidden bg-white text-left w-full transition-colors">
                ...
                <div className="grid grid-cols-2 divide-x divide-gray-400">
                  ...
                  <button
                    ...
                    className="p-2.5 sm:p-3 text-left hover:bg-gray-50 transition-colors cursor-pointer w-full focus:outline-none"
                  >
                    <div className="text-[10px] font-bold tracking-wider text-gray-700 uppercase">
                      CHECK IN
                    </div>
                    <div className="text-sm font-semibold text-gray-900 mt-0.5 truncate">
                      {checkInDate ? (
                        ...
                        <span className="text-gray-400 font-normal">Add date</span>
                      )}
                    </div>
                  </button>
                ...
                <div className="border-t border-gray-400 relative">
                  <button
                    ...
                    className="w-full p-2.5 sm:p-3 text-left hover:bg-gray-50 transition-colors cursor-pointer flex items-center justify-between focus:outline-none"
                  >
                    <div>
                      <div className="text-[10px] font-bold tracking-wider text-gray-700 uppercase">
                        GUEST
                    ...
                    <ChevronDown className={`w-4 h-4 text-gray-700 transition-transform duration-200 ${showGuestDropdown ? "rotate-180" : ""}`} />
                  </button>

                  {/* Inline Guest Popover Dropdown */}
                  {showGuestDropdown && (
                    <div className="p-4 bg-white border-t border-gray-200 shadow-lg space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            disabled={adults <= 1}
                            className="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:border-gray-900 disabled:opacity-30 cursor-pointer"
                          >
                            <Minus className="w-3.5 h-3.5 text-gray-700" />
                          </button>
                          <span className="w-6 text-center font-bold text-sm text-gray-900">{adults}</span>
                          <button
                            type="button"
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="w-6 text-center font-bold text-sm text-gray-900">{children}</span>
                          <button
                            type="button"
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() => onGuestChange(adults, children, Math.max(0, (infants || 0) - 1))}
                            disabled={!infants || infants <= 0}
                            className="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:border-gray-900 disabled:opacity-30 cursor-pointer"
                          >
                            <Minus className="w-3.5 h-3.5 text-gray-700" />
                          </button>
                          <span className="w-6 text-center font-bold text-sm text-gray-900">{infants || 0}</span>
                          <button
                            type="button"
                            onClick={() => onGuestChange(adults, children, (infants || 0) + 1)}
                            className="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center hover:border-gray-900 cursor-pointer"
                          >
                            <Plus className="w-3.5 h-3.5 text-gray-700" />
                          </button>
                        </div>
                      ...
                      <div className="pt-2 border-t border-gray-100 flex justify-between items-center text-xs text-gray-500">
                        <span>Max {villa?.guests || 4} guests</span>
                        <button
                          type="button"
                          onClick={() => setShowGuestDropdown(false)}
                          className="text-gray-900 font-semibold underline hover:text-[#D4AF37] cursor-pointer"
                        >
                          Close
                        </button>
                      </div>
              ...
              {/* Standard Check-in/Check-out Info Trigger */}
              <div className="flex items-center justify-center pt-2">
                <button
                  type="button"
                  onClick={() => setShowTimingInfo(true)}
                  className="inline-flex items-center gap-1.5 text-xs text-gray-600 hover:text-[#D4AF37] transition-all py-1.5 px-3 rounded-lg hover:bg-gray-100 border border-gray-200 cursor-pointer group"
                >
                  <Info className="w-3.5 h-3.5 text-[#D4AF37] group-hover:scale-110 transition-transform" />
                  <span className="font-medium">Standard Check-in/Check-out Times</span>
                </button>
              </div>

              {/* Timing Info Popup Modal */}
              {showTimingInfo && (
                <div
                ...
                >
                  <div
                    className="bg-white rounded-2xl max-w-md w-full p-5 sm:p-6 border border-gray-100 shadow-2xl relative"
                  ...
                  >
                    <div className="flex items-center justify-between pb-3 border-b border-gray-100 mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="font-bold text-gray-900 text-base">Check-in & Check-out Policy</h4>
                      </div>
                      <button
                        type="button"
                        onClick={() => setShowTimingInfo(false)}
                    ...
                    <div className="space-y-3 text-sm text-gray-700">
                      <div className="p-3.5 rounded-xl bg-gray-50 border border-gray-200 flex items-start gap-3">
                        ...
                        <div>
                          <span className="font-semibold text-gray-900 block">Check-in Time</span>
                          <span className="text-gray-600">14:00 (2:00 PM) on arrival day</span>
                        </div>
                      </div>
                      ...
                      <div className="p-3.5 rounded-xl bg-gray-50 border border-gray-200 flex items-start gap-3">
                        ...
                        <div>
                          <span className="font-semibold text-gray-900 block">Check-out Time</span>
                          <span className="text-gray-600">12:00 (12:00 PM) on departure day</span>
                        </div>
                      </div>

                      <p className="text-xs text-gray-500 pt-1 leading-relaxed">
                        When you select dates {checkInDate && checkOutDate && `(${totalNights} nights)`}, your stay includes full access to all selected days and private amenities.
                      </p>
                    </div>
                    ...
                    <div className="mt-5 pt-3 border-t border-gray-100">
                      <button
                        type="button"
                        onClick={() => setShowTimingInfo(false)}
                        className="w-full py-2.5 bg-gray-900 hover:bg-black text-white rounded-xl text-sm font-semibold transition-colors cursor-pointer"
                      >
                        Got it
                      </button>
                    </div>
          ...
          {bookingStep === 2 && (
            <div className="space-y-4">
              <div className="bg-gradient-to-r from-[#D4AF37]/10 to-[#BFA181]/10 rounded-xl p-4 border border-[#D4AF37]/20">
                ...
                <div className="mt-3 p-2 bg-white rounded-lg text-center border border-gray-200">
